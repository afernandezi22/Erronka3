...
END;
/
--BEZEROAK EZABATU AHAL IZATEKO
ALTER TABLE BEZERO_TELEFONO DROP CONSTRAINT FK_BEZ;

ALTER TABLE BEZERO_TELEFONO ADD CONSTRAINT FK_BEZ FOREIGN KEY (ID_BEZERO) REFERENCES BEZERO(ID) ON DELETE CASCADE;
/
...
CREATE OR REPLACE FUNCTION ERABAKIBILTEGIA(PRODUKTUID PRODUKTU.ID%TYPE) RETURN NUMBER AS
    BILTEGIA NUMBER; 
BEGIN
    SELECT I.ID_BILTEGI INTO BILTEGIA FROM PRODUKTU P, INBENTARIO I WHERE P.ID = PRODUKTUID AND P.ID = I.ID_PRODUKTU group by P.ID, I.ID_BILTEGI ORDER BY MAX(I.KOPURUA) DESC FETCH FIRST 1 ROWS ONLY;    
    RETURN BILTEGIA;
END;
/
...
/
--EROSKETA PROZESUA 
CREATE OR REPLACE PROCEDURE EROSKETAPROZEDURA (PRODUKTUKODEA PRODUKTU.ID%TYPE, ERABILTZAILEA BEZERO.EMAILA%TYPE) AS
    BILTEGIA NUMBER;
    PRODUKTUA PRODUKTU.ID%TYPE;
    IDMAX ESKARI.ID%TYPE;
    PREZIOA PRODUKTU.SALNEURRIA%TYPE;
    IDBEZERO BEZERO.ID%TYPE;
    VIPBEZ BEZERO.VIP%TYPE;
    KATPROD PRODUKTU.ID_KATEGORIA%TYPE;
    S NUMBER(4); --Stocka zehazteko kasu bakoitzean
BEGIN
    STOCK(PRODUKTUKODEA ,S);
    IF S = 0 THEN
        RAISE_APPLICATION_ERROR(-20004, 'Ez dago stockik');
    END IF;
    
    --INBENTARIOTIK KENTZEKO
    BILTEGIA := ERABAKIBILTEGIA(PRODUKTUKODEA);
    ...
    SELECT ID INTO IDBEZERO FROM BEZERO WHERE EMAILA = ERABILTZAILEA;
    INSERT INTO ESKARI(ID, ID_BEZERO, ID_EGOERA, ID_SALTZAILE, ESKAERA_DATA) VALUES (IDMAX+1, IDBEZERO, 1, NULL, SYSDATE);
    
    --ESKARI LERROA SORTZEKO
    SELECT SALNEURRIA INTO PREZIOA FROM PRODUKTU WHERE ID = PRODUKTUKODEA;
    
     --VIP DESKONTUA DUEN EDO EZ JAKITEKO
    SELECT VIP INTO VIPBEZ FROM BEZERO WHERE EMAILA = ERABILTZAILEA;
    SELECT ID_KATEGORIA INTO KATPROD FROM PRODUKTU WHERE ID = PRODUKTUKODEA;
    
    IF VIPBEZ = KATPROD THEN
        PREZIOA := PREZIOA * 0.75;
    END IF;
...
END;
/
--8 PANTAILA. Egoera aldaketak
CREATE OR REPLACE TRIGGER EGOERALDAKETA
AFTER UPDATE OF ID_EGOERA ON ESKARI FOR EACH ROW
BEGIN
    INSERT INTO EGOERA_ALDAKETAK(ESKARI_ID, SALTZAILE_ID, EGOERA_ZAHARRA, EGOERA_BERRIA, DATA) VALUES (:NEW.ID, :NEW.AZKEN_ALDAKETA, :OLD.ID_EGOERA, :NEW.ID_EGOERA, SYSDATE);
END;
/
